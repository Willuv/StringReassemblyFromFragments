...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
         ...
         */
        //create a string that takes the first string up to the point of overlap
        //and combines it with the entire second string
        String str = str1.substring(0, str1.length() - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        //boolean variable to determine whether the string is a substring in the set
        boolean contain = false;
        for (String iterate : strSet) {
            if (iterate.contains(str)) {
                contain = true;
            }
        }

        //if str isn't a substring in any
        if (!contain) {
            //add it to the set
            strSet.add(str);
            //create a new set
            Set<String> si = new Set1L<>();
            //iterate through the set
            for (String iterate : strSet) {
                //if the current iteration of set is equal to the string add
                //it to the temp set
                if (iterate.equals(str)) {
                    si.add(iterate);
                } else {
                    //else as long as the iteration doesn't contain str add it to temp
                    if (!str.contains(iterate)) {
                        si.add(iterate);
                    }
                }
            }
            //clear strSet and transfer from the temp set si
            strSet.clear();
            strSet.add(si);
        }
    }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        // while the end of the input isn't reached, add each line to the set of lines
        while (!input.atEOS()) {
    ...
    private static int bestOverlap(Set<String> strSet, String[] bestTwo) {
        ...
        //missing precondition for there are no substring pairs
        Set<String> test = strSet.newInstance();
        test.transferFrom(strSet);
        while (test.size() > 0) {
            String x = test.removeAny();
            strSet.add(x);
    ...
    public static void assemble(Set<String> strSet) {
        ...
        //precondition to check if there are pairs of substrings
        Set<String> test = strSet.newInstance();
        test.transferFrom(strSet);
        while (test.size() > 0) {
            String x = test.removeAny();
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < text.length(); i++) {
            //is the ~ is found print a new line
            if (text.charAt(i) == '~') {
                out.println();
            } else {
                //else just print the current char
